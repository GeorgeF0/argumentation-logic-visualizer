...
\end{forest}}
\caption{Visualization of the proof of example in \autoref{sec:vizalg}\label{fig:vizex}}
\end{figure}
...
When considering the formulas of the conjunction that constitutes an attack, all formulas that are part of the theory or the argument under attack are ignored. Absence of any remaining formulas signifies an attack by the empty set. This is shown by the last two examples in \autoref{sec:vizex}.

Although the argumentation framework shown in \autoref{fig:vizex} could have been drawn so that argument $\{\beta\}$ on the left side of the ramification attacks $\{\neg\beta\}$ on the right, the algorithm repeats the whole branch in the drawing. The choice was merely based on visual appearance and on the fact that trees can be formed this way. The algorithm could be made instead to draw an attack from $\{\beta\}$ on the left branch to $\{\gamma\}$ on the right.
...
\end{forest}}
\end{minipage}%
\caption{Visualization example of empty set attack}
\end{figure}
